...
The local Rust HTTP adapter lives in `platform-web`; `platform-service` now composes it with SQLite authentication. Service units, HTTPS configuration and image instructions are in `platform-buildroot`. The SSR bundle includes its React dependencies and can be staged without `node_modules`. The renderer is not a substitute for the independent `/recovery` route.

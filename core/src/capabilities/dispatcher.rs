...
impl CapabilityDispatcher {
    ...
    > {
        if !capability_registry
            .is_registered(
                invocation.capability_provider()
            )
        {
            return Err(
                CapabilityDispatchError::
        ...
        Ok(
            authorization_engine.authorize(
                &authorization_request,
                grants,
            )
        )
    ...
    fn fmt(
        ...
        match self {
            ...
            Self::AccessRuleMissing => {
                write!(
                    f,
                    "capability has no registered access rule"
                )
            }

...
use crate::{AuthorizationRequest, PermissionId, ResourceRef};

use super::{CapabilityId, CapabilityInvocation};
...
impl CapabilityAccessRule {
    pub fn new(capability: CapabilityId, permission: PermissionId) -> Self {
        Self {
            capability,
            permission,
        }
    ...
    ) -> Result<AuthorizationRequest, CapabilityAccessRuleError> {
        if invocation.capability() != &self.capability {
            return Err(CapabilityAccessRuleError::CapabilityMismatch);
        }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::CapabilityMismatch => {
                write!(f, "capability invocation does not match access rule")
            }
        }
    }
}
...
mod tests {
    ...
    use crate::{
        AppId, AppIdentity, CapabilityProvider, InstallationId, OperationContext, PublisherId,
        ResourceKey, ResourceKind, ResourceNamespace,
    };
    ...
    fn creates_authorization_request_for_matching_capability() {
        ...
        let provider = app("com.rumahl.files");

        let capability = CapabilityId::parse("rumahl.files.preview").unwrap();

        let capability_provider =
            CapabilityProvider::new(provider.into(), capability.clone()).unwrap();
        ...
        let rule = CapabilityAccessRule::new(
            capability,
            PermissionId::parse("rumahl.files.read").unwrap(),
        );

        let result = rule.authorization_request(&invocation, Some(file("document-1")));
    ...
    fn rejects_rule_for_different_capability() {
        ...
        let invocation_capability = CapabilityId::parse("rumahl.files.preview").unwrap();

        let provider = CapabilityProvider::new(provider.into(), invocation_capability).unwrap();

        let invocation =
            CapabilityInvocation::new(OperationContext::for_background_app(consumer), provider);

        let wrong_rule = CapabilityAccessRule::new(
        ...
        assert_eq!(
            wrong_rule
                .authorization_request(&invocation, None,)
                .unwrap_err(),

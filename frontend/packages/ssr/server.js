...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    response.writeHead(200, {
      ...
      "Content-Security-Policy": `default-src 'none'; script-src 'self' 'nonce-${payload.nonce}'; style-src 'self'; img-src 'self' data:; connect-src 'self'; frame-src ${frameOrigins.length ? frameOrigins.join(" ") : "'none'"}; base-uri 'none'; object-src 'none'; frame-ancestors 'none'`,

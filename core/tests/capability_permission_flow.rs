use rumahl_core::{
    ...
    CapabilityAccessRegistry, CapabilityAccessRule, CapabilityId, CapabilityInvocation,
    CapabilityProvider, CapabilityRegistry, GrantAuthority, GrantIssuerPolicy, InMemoryGrantStore,
    InstallationId, OperationContext, PermissionId, PermissionScope, PublisherId, ResourceKey,
    ResourceKind, ResourceNamespace, ResourceRef, SessionId, UserId, UserIdentity, UserRole,
};
...
fn capability_invocation_is_authorized_through_permission_engine() {
    ...
    let invocation = CapabilityInvocation::new(
        OperationContext::for_app_as_user(consumer, user_id, SessionId::new()),
        resolved_provider,
    );
    ...
    let resolved_rule = access_registry.rule_for(&capability).unwrap();
    ...
    let allowed_request = resolved_rule
        .authorization_request(&invocation, Some(allowed_file))
        .unwrap();

    assert_eq!(
        engine.authorize(&allowed_request, store.grants(),),
    ...
    let denied_request = resolved_rule
        .authorization_request(&invocation, Some(denied_file))
        .unwrap();

    assert_eq!(
        engine.authorize(&denied_request, store.grants(),),
